module UsersHelper
  ...
  private
    ...
    def users_by_points limit=nil, offset=nil
      ...
      sql = "SELECT users.id, users.created_at, #{points} as total FROM users #{fans} WHERE roles.name != 'admin' OR roles.id IS NULL ORDER BY total DESC NULLS LAST, users.created_at ASC, users.id ASC"
